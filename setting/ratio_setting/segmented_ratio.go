...
type SegmentRule struct {
	...
	InputMin int `json:"input_min"` // Minimum input tokens (inclusive), 0 means no lower bound
	InputMax int `json:"input_max"` // Maximum input tokens (inclusive), 0 means no upper bound

	// Output token constraints (in tokens, not 1K tokens)
	OutputMin int `json:"output_min"` // Minimum output tokens (inclusive), 0 means no lower bound
	OutputMax int `json:"output_max"` // Maximum output tokens (inclusive), 0 means no upper bound
...
func matchesSegmentRule(rule SegmentRule, inputTokens, outputTokens int) bool {
	...
	if rule.InputMin > 0 && inputTokens < rule.InputMin {
		return false
	}
	if rule.InputMax > 0 && inputTokens > rule.InputMax {
		return false
	}

	// Check output token constraints
	if rule.OutputMin > 0 && outputTokens < rule.OutputMin {
		return false
	}
	if rule.OutputMax > 0 && outputTokens > rule.OutputMax {
		return false
	}
